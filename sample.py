...
parser.add_argument('--model', type=str,
                    default='reinforce_cartpole/model.pt')  # model - currently supports resnet and alexnet, with more to come
...
class Runner():
    ...
    def train_inner(self):
        ...
        has_next_state = torch.tensor(list(map(lambda s: s is not None, batch.next_state)), device=device,
                                      dtype=torch.bool)
    ...
    def train(self, episodes=100, smooth=10):
        ...
        for episode in range(episodes):
            ...
            if episode % 20 == 0:
                print("\tEpisode {} \t Final Reward {:.2f} \t Average Reward: {:.2f}".format(episode, rewards,
                                                                                             np.mean(smoothed_reward)))
